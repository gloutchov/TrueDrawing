...
- enter or remove the OpenAI API key from `File > Settings > API Key...`;
- type the OpenAI image model from the same settings dialog;
- choose the image style from `File > Settings > Style...`, using predefined styles or a custom value;
- enable inspector auto-redraw from `File > Settings > Auto redraw...`;
- choose interface language and theme from `File > Settings > Interface...`, including system-default options;
...
The API key must not be placed in that file: it is entered from the app through `File > Settings > API Key...` and stored by the Electron main process in Windows Credential Manager on Windows, macOS Keychain on macOS, or encrypted local fallback storage on unsupported environments.
